...
public class ProRead {
    public static void write() {
        try {
            ...
            prop.setProperty("noder",node);
            prop.setProperty("corer",core);
            prop.setProperty("sd",ShareDatabase);
    ...
    public static void crNew() {
        try {
            Properties prop = new Properties();
            prop.setProperty("all_connect", "");
            prop.setProperty("port", "");
            prop.setProperty("noder","");
            prop.setProperty("corer","");
            prop.setProperty("sd","");
            prop.store(new FileOutputStream(
                            new File("." + File.separator + "apsql" + File.separator + "config" + File.separator + "info.properties"), Boolean.parseBoolean("utf-8")),
                    "Properties For SQL");
