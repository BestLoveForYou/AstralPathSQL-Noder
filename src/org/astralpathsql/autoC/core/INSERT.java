...
public class INSERT {
    public static String in(String sp[],String org) {
        ...
        if (sp[1].equals("into")) {
            ...
            if (sp[3].equals("values")) {
                ...
                if (database.toString().contains(ShareDatabase)) {
                    String nodea[] = node.split(",");
                    Map<String,Noder> mas = new HashMap<>();
                    Map<String,Noder> maf = new HashMap<>();
                    int succ = 1;
                    for (int xaa = 0; xaa < nodea.length; xaa++) {
                        try {
                            Noder n = noder_pool.get(nodea[xaa].toString());
                            int a = n.nodecmd("+ " + String.valueOf(c));
                            succ = succ + a;
                            if (a==-1) {
                                maf.put(nodea[xaa],n);
                            } else {
                    ...
                    System.out.println(1);
                    if (succ > 0) {
                        Set<Map.Entry<String,Noder>> entries1 = maf.entrySet();
                        for (Map.Entry entry:entries1){
                            try {
                                Noder n = noder_pool.get(entry.getKey().toString());
                                String de = "delete from " + t.getName() + " where id = " + c.getId();
                                n.nodecmd("- "  + de);
                                n.nodecmd("+ " + String.valueOf(c));

                            } catch (Exception e) {}
                        }
                    } else {
                        Set<Map.Entry<String,Noder>> entries1 = mas.entrySet();
                        for (Map.Entry entry:entries1){
                            try {
                                Noder n = noder_pool.get(entry.getKey().toString());
                                String de = "delete from " + t.getName() + " where id = " + c.getId();
